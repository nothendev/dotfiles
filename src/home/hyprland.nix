...
in
{
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      (lib.hm.assertions.assertPlatform "wayland.windowManager.hyprlandy" pkgs lib.platforms.linux)
    ];

    warnings =
      let
        inconsistent =
          (cfg.systemdIntegration || cfg.plugins != [ ]) && cfg.extraConfig == "" && cfg.settings == { };
        warning = "You have enabled hyprland.systemdIntegration or listed plugins in hyprland.plugins but do not have any configuration in hyprland.settings or hyprland.extraConfig. This is almost certainly a mistake.";
      in
    ...
    xdg.configFile."hypr/hyprland.conf" =
      let
        ...
        shouldGenerate = cfg.systemdIntegration || cfg.extraConfig != "" || combinedSettings != { };
        ...
        toHyprconf =
          ...
          let
            ...
            importantFields = filterAttrs (n: _: (hasPrefix "$" n) || (hasPrefix "bezier" n)) allFields;
      ...
      lib.mkIf shouldGenerate {
        text =
          ...
          ''
          + lib.optionalString (combinedSettings != { }) (toHyprconf combinedSettings 0)
